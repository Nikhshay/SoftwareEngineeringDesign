...
void WriteDutyCycle(unsigned int val)
{
	char command[256] = { "ousb pwm 1 " }; //small change
	FILE *fpipe;
	char line[250] = {};
	char val_string[4] = {};//write
	_itoa_s(val, val_string, 10);//write
	strcat_s(command, val_string);//write
...
int ReadADC(unsigned int val)
{
	...
	unsigned int adcvalue = (unsigned int)atoi(line); // convert from char array to unsigned short
	return adcvalue;
}

bool checkrange(int num, string line)
{
	if (line == "overall" && num >= 0 && num <= 1023)
	{
		return true;
	}
	else if (line == "P" && num >= 0 && num <= 100)
	{
		return true;
	}
	else if (line == "A" && num >= 0 && num <= 7)
	{
		return true;
	}
	else
...
int main(int argc, char *argv[])
{
	...
	else if (argc == 3)
	{
		...
		string checksecond = argv[2];
		int secondargument = stoi(argv[2]);
		for (int i = 0; i < checksecond.length(); i++)
		{
			if (isdigit(checksecond[i]));
			else
			{
				secondargument = 1500;
			}
		}
		if (checkrange(secondargument, "overall") == false)
		{
			cout << "R" << endl;
			return 0;
		}
		...
		if (argv[1][0] == 'P')
		{
			if (checkrange(secondargument, "P") == false)
			{
				cout << "r" << endl;
				return 0;
			}
			...
			SetFrequency();
			WriteDutyCycle(secondargument);
			return 0;
		}
		else if (argv[1][0] == 'A')
		{
			if (checkrange(secondargument, "A") == false)
			{
				cout << "a" << endl;
				return 0;
			}
			else;
			int portA_value = ReadADC(secondargument);
			cout << portA_value << endl;
			return 0;
		}
		else if (argv[1][0] == 'L')
		{
			SetFrequency();
			int targetadc = secondargument;
			int currentpwm = 50;
		...
		else if (argv[1][0] == 'M')
		{
			...
			{
				Sleep(10);
				cout << "Loop number " << i << " Current PWM is " << currentpwm << " Current ADC is " << currentADC << " Target ADC is " << secondargument << endl;
				currentADC = ReadADC(0);
				if (currentADC > targetadc)
				{
					if (currentpwm != 100)
				...
				else if (currentADC < targetadc)
				{
					...
					{
						cout << "Motor at Min Speed (Stopped)" << endl;
						Sleep(1000);
					}
				}
				else
