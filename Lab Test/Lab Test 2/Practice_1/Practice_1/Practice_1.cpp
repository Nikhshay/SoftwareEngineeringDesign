...
bool checkrange(string line, int val)
{
	if (line == ":)")
	{
		if (val >= 0 && val <= 1023)
...
int main(int argc, char *argv[])
{
	...
	else if (argc == 3)
	{
		...
		{
			cout << "incoorect 2nd argument (I)" << endl;
			return 0;
		}
		int num = atoi(argv[2]);
		if (checkrange(":)", num));
		...
		{
			cout << "Incorrect input range (R)" << endl;
			return 0;
		}
		...
		else if (argv[1][0] == 'A')
		{
			...
			{
				cout << "out of range (a)" << endl;
				return 0;
			}
			cout << ReadADC(num) << endl;
		...
		else if (argv[1][0] == 'L')
		{
			...
			if (checkrange("L", num))
			{
				cout << "Below printing for debugging" << endl;
					...
					for (int i = 1; i <= 60; i++)
					{
						...
						WriteDutyCycle(currentpwm);
						cout << "Loop number #" << i << " ADC target = " << targetadc << " ADC Read " << adc << " Next PWM " << currentpwm << endl;
					}
					cout << "Final PWM written to ousb is " << currentpwm << " Final ADC acheived is " << adc << endl;
					cout << "The only value to be printed during the test is (" << currentpwm << ")" << endl;
			}
		...
		else if (argv[1][0] == 'M')
		{
			...
			{
				WriteDutyCycle(100);
				int maxspeedADCvalue = ReadADC(0);	//approx 60
				WriteDutyCycle(0);
				int minspeedADCvalue = ReadADC(0);  //approx 288
				cout << "Duty cycle = 100, ADC value = " << maxspeedADCvalue << endl;
				cout << "Duty cycle = 0, ADC value = " << minspeedADCvalue << endl;
				for (int i = 1; i > 0; i++)
				{
					adc = ReadADC(0);
					if (adc > targetadc)
	...
	{
		cout << "Too many/little argument (P)" << endl;
		return 0;
	}

			// The convention is to return Zero to signal NO ERRORS, if you change it 
