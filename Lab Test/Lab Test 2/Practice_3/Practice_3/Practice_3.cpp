...
bool checkrange(int number, string when)
{
	if (when == ":)")
	{
...
int main(int argc, char *argv[])
{
	...
	else if (argc == 3)
	{
		...
		if(argv[1][0] == 'P')
		{
			if (checkrange(secondarg, "P"))
			{
				SetFrequency();
				WriteDutyCycle(secondarg);
				cout << "p" << endl;
				return 0;
			}
			else
		...
		{
			if (checkrange(secondarg, "A"))
			{
				int portvalue = ReadADC(secondarg);
				cout << portvalue << endl;
				return 0;
			}
			else
		...
		else if (argv[1][0] == 'M')
		{
			...
			for (int i = 1; i > 0; i++)
			{
				cout << " Current PWM is " << currentpwm << " Current ADC is " << currentadc << " Target ADC is " << targetADC << endl;
				if (targetADC > currentadc && currentpwm > 0)
				{
					currentpwm--;
				}
