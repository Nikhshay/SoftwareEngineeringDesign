...
int main(int argc, char *argv[])
{
	...
	else if (argc == 2)
	{
		...
		{
			OUSB board;
			int val = board.read_write_port("r", charportb, 0);
			cout << val << endl;
			return 0;
		}
		else if (argv[1][0] == 'D')
		{
			OUSB board;
			int val = board.read_write_port("r", charportd, 0);
			cout << val << endl;
			return 0;
		}
		else
	...
	else if (argc == 3)
	{
		...
		{
			OUSB board;
			int val = board.read_write_port("w", charportd, argument2);
			val = board.read_write_port("r", charportd, 0);
			cout << val << endl;
			return 0;
		}
		...
		{
			OUSB board;
			int currentportb = board.read_write_port("r", charportb, 0);
			int currentportd = board.read_write_port("r", charportd, 0);
			if (argument2 >= 0 && argument2 <= 15)
			{
				TrafficLight nik;
				for (int i = 0; i < argument2; i++)
				{
					nik.changeTrafficLightState(argv[1][0]);
					switch (argv[1][0])
					{
					case 'R':
					{
						currentportb = (currentportb & 240) + 1;
						board.read_write_port("w", charportb, currentportb);
						currentportd = (currentportb & 207) + 16;
						board.read_write_port("w", charportd, currentportd);
					...
					case 'Y':
					{
						currentportb = (currentportb & 240) + 6;
						board.read_write_port("w", charportb, currentportb);
						currentportd = (currentportb & 207) +32;
						board.read_write_port("w", charportd, currentportd);
					...
					case 'G':
					{
						currentportb = (currentportb & 240) + 8;
						board.read_write_port("w", charportb, currentportb);
						currentportd = (currentportb & 207);
						board.read_write_port("w", charportd, currentportd);
				...
				cout << currentportb << "," << currentportd;
				return 0;
			}
