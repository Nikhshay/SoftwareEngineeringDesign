...
Proforma version number: 2019.001.e (updated 23-05-2019)

Document Revision Details:
version 2019.001.a (11-04-2019) - Original version
version 2019.001.b (03-05-2019) - G13 updated
version 2019.001.c (16-05-2019) - Example 11 updated
version 2019.001.d (17-05-2019) - Extra comments add to the OUSB and TrafficLightclass class code
version 2019.001.e (23-05-2019) - Adjusted comments indicating where class methods should be
								  declared and defined within this file. see NOTE 1 and NOTE 2
...
{
	char command[256] = {}//incomplete project;
	...
	FILE *fpipe;
	char line[250] = {}//incomplete project; // size of Line should be smaller than command
...
public:
	OUSB() { PORTB = PORTD = 0; }//incomplete project  // Constructor: Remember to define this else it won't compile

	 // Example of Interface methods (These can be changed....)
	int r_w_ports(string r_w, char ports[], int val);
...
int OUSB::r_w_ports(string r_w, char ports[], int val)
{
	string port = ports;
	char command[256] = {}//incomplete project;
	strcpy_s(command, "ousb -r io "); // command to write 20 to OUSB board
	char val_string[4] = {}//incomplete project;
	...
	if (r_w == "w")
	{
		if (port == "PORTB")
		{
			strcat_s(command, "PORTB ");
			strcat_s(command, val_string);
		}//incomplete project
		else if (port == "PORTD")
		{
			strcat_s(command, "PORTD ");
			strcat_s(command, val_string);
		}//incomplete project
		else;
	...
	if (fpipe != NULL)					   // check that the pipe opened correctly
	{
		...
		if (port == "PORTB")
		{
			PORTB = (int)atoi(line); // convert from char array to int;
			return PORTB;
		}//incomplete project
		else if (port == "PORTD")
		{
			PORTD = (int)atoi(line); // convert from char array to int;
			return PORTD;
		}//incomplete project
		else;
...
private:
	...
	bool green;
public:
	TrafficLight() {red = yellow = green = false ; }//incomplete project // Constructor: Remember to define this else it won't compile
...
char charportb[] = { "PORTB" }//incomplete project;
char charportd[] = { "PORTD" }//incomplete project;
int portbval = 0;
int portdval = 0;
...
int main(int argc, char *argv[])
{
	...
	else if (argc ==2)
	{
		...
		{
			OUSB board;
			int val = board.r_w_ports("r", charportb, 0);
			cout << val << endl;
			return 0;
		}//incomplete project
		else if (argv[1][0] == 'D')
		{
			OUSB board;
			int val = board.r_w_ports("r", charportd, 0);
			cout << val << endl;
			return 0;
		}//incomplete project
		else
	...
	else if (argc ==3)
	{
		...
		else if (argv[1][0] == 'C' && (argv[1][1] == 'B' || argv[1][1] == 'D' || argv[1][1] == 'R' || argv[1][1] == 'Y' || argv[1][1] == 'G') && argv[1][2] == '\0');
		else
		{
		...
		if (argv[1][0] == 'B')
		{
			...
			{
				OUSB board;
				int val = board.r_w_ports("w", charportb, argument2);
				cout << val << endl;
				return 0;
			}//incomplete project
			else
		...
		else if (argv[1][0] == 'D')
		{
			...
			{
				OUSB board;
				board.r_w_ports("w", charportd, argument2);
				int val = board.r_w_ports("r", charportd, 0);
				cout << val << endl;
				return 0;
			}//incomplete project
		...
		else if(argv[1][0] != 'C')
		{
			if (argument2 >= 0 && argument2 <= 255)
			{
				TrafficLight ckm;
				OUSB board;
				portbval = board.r_w_ports("r", charportb, 0);
				portbval = board.r_w_ports("r", charportd, 0);
				for (int i = 0; i <= argument2; i++)
				{
					ckm.changeTrafficLightState(argv[1][0]);
					switch (argv[1][0])
					{
					case 'R':
					{
						portbval = (portbval & 240) + 1;//adding wrong
						board.r_w_ports("w", charportb, portbval);

						portdval = (portdval & 207) + 16;//adding wrong
						board.r_w_ports("w", charportd, portdval);
					...
					case 'Y':
					{
						portbval = (portbval & 240) + 6;//adding wrong
						board.r_w_ports("w", charportb, portbval);

						portdval = (portdval & 207) + 32;//adding wrong
						board.r_w_ports("w", charportd, portdval);
					...
					case 'G':
					{
						portbval = (portbval & 240) + 8;//adding wrong
						board.r_w_ports("w", charportb, portbval);
				...
				portbval = board.r_w_ports("r", charportb, 0);
				portdval = board.r_w_ports("r", charportd, 0);
				cout << portbval << "," << portdval;
			}//incomplete project
			else
			{
				cout << "Out of range" << endl;
