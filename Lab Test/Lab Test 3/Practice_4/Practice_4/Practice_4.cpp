...
using namespace std; // Please do not remove this line
					 ...
					 // optional: your choice if you want to use/modify these definitions for MAX-MIN-R
...
void TrafficLight::changeTrafficLightState(char *state)
{
	...
	if (*state == 'R')
	{
		...
		portbval = (portbval & clear) + 1 + count;
	...
	else if (*state == 'Y')
	{
		...
		portbval = board.read_write_ports("r", charportb, 0);
		portbval = (portbval & clear) + 6 + count;
		board.read_write_ports("w", charportb, portbval);
	...
	else if(*state == 'G')
	{
		...
		portbval = board.read_write_ports("r", charportb, 0);
		portbval = (portbval & clear) + 8 + count;
		board.read_write_ports("w", charportb, portbval);
