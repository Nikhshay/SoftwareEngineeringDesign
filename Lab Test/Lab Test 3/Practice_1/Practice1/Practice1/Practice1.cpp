...
private:
	...
	int currentstate = 0;
public:
	TrafficLight() { redLamp = yellowLamp = greenLamp = false; }
	void changeTrafficLightStateRGY(char state);
	void changeTrafficLightStateCRGY(char state, int count);

};



void TrafficLight::changeTrafficLightStateRGY(char state)
{
	OUSB board;
	char char_b[] = { "PORTB" };
	char char_d[] = { "PORTD" };
	...
	int portb_val = board.read_write_port("r", char_b, 0);
	int portd_val = board.read_write_port("r", char_d, 0);
	if (state == 'R')
	{
		redLamp = true;
		yellowLamp = false;
		...
		portb_val = (portb_val & 240) + 1;
		board.read_write_port("w", char_b, portb_val);
		portd_val = (portd_val & 207) + 16;
		board.read_write_port("w", char_d, portd_val);
		//Sleep(1000);
		return;
	}
	else if (state == 'Y')
	{
		...
		portb_val = (portb_val & 240) + 6;
		board.read_write_port("w", char_b, portb_val);
		portd_val = (portd_val & 207) + 32;
		board.read_write_port("w", char_d, portd_val);
		//Sleep(1000);
		return;
	}
	else
	{
		...
		portb_val = (portb_val & 240) + 8;
		board.read_write_port("w", char_b, portb_val);
		portd_val = (portd_val & 207);
		board.read_write_port("w", char_d, portd_val);
...
void TrafficLight::changeTrafficLightStateCRGY(char state, int count)
{
	OUSB board;
	char char_b[] = { "PORTB" };
	char char_d[] = { "PORTd" };
...
int main(int argc, char *argv[])
{
	...
	if (argc == 2)//1 argument
	{
		...
		{
			OUSB ousb;
			int val = ousb.read_write_port("r", char_b, 0);
			if (val == 6969)
			{
				cout << "Z" << endl;
				return 0;
		...
		else if (argv[1][0] == 'D')
		{
			...
			OUSB ousb;
			int val = ousb.read_write_port("r", char_d, 0);
			if (val == 6969)
			{
				cout << "Z" << endl;
				return 0;
	...
	else if (argc == 3)
	{
		...
		int int_argument2 = atoi(argv[2]);//second argument as int
		...
		int portb_val = board.read_write_port("r", char_b, 0);
		int portd_val = 0;
		if (portb_val == 6969)//check hardware disconnection
		{
			cout << "Z" << endl;
			return 0;
		}
		...
		if (argv[1][0] == 'B')
		{
			if (int_argument2 >= 0 && int_argument2 <= 255)
			{
				board.read_write_port("w", char_b, int_argument2);
				portb_val = board.getPORTBvalue();
				cout << portb_val << endl;
				return 0;
			}
			else
		...
		else if (argv[1][0] == 'D')
		{
			if (int_argument2 >= 0 && int_argument2 <= 255)
			{
				board.read_write_port("w", char_d, int_argument2);
				portd_val = board.read_write_port("r", char_d, 666);
				cout << portd_val << endl;
				return 0;
			}
			else
		...
		else if (argv[1][0] == 'R' || argv[1][0] == 'G' || argv[1][0] == 'Y')
		{
			...
			if (int_argument2 >= 0 && int_argument2 <= 15)
			{
				for (int i = 0; i <= int_argument2; i++)
				{
					Shibuya_crossing.changeTrafficLightStateRGY(argv[1][0]);
					if (argv[1][0] == 'G')
					{
						argv[1][0] = 'Y';
					}
				...
				portb_val = board.read_write_port("r", char_b, 0);
				portd_val = board.read_write_port("r", char_d, 0);
				cout << portb_val << "," << portd_val;
				return 0;
			}
			else
			{
		...
		else
		{
			...
			if (int_argument2 >= 0 && int_argument2 <= 15)
			{
				for (int i = 0; i <= int_argument2; i++)
				{
					Shibuya_crossing.changeTrafficLightStateCRGY(argv[1][1], count);
					if (argv[1][1] == 'G')
					{
						argv[1][1] = 'Y';
					}
