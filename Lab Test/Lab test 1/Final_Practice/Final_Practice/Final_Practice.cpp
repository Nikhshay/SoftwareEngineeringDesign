...
int main(int argc, char *argv[])
{
	...
	else if (argc == 3)
	{
		...
		int hcounter = 1;
		int vcounter = 1;
		ofstream outfile(argv[2], ios::app | ios::out);
		int i = 0;
		/*while (!infile.eof())
		{
			hcounter++;
			...
			if (hcounter >= 300 && hcounter <= 450 && vcounter >= 150 && vcounter <= 300)
			{
				outfile << i<< ":pix_val = 24'h" << line << ";" << endl;
			}
		}
		cout << hcounter << "and" << vcounter << endl;*/
		while (i < 650000)
		{
			i++;
			getline(infile, line);
